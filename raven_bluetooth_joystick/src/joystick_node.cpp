...
bool ShouldPublishMessages() {
  return some_button_changed_state || (message.axis0_lr != 0) ||
         (message.axis0_ud != 0);
}
...
void CaptureJoystickEvent() {
  ...
  close(js);
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] device loop shutdown");
}
...
void PublishMessages() {
  ...
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] messages publisher shutdown");
}

int main(int argc, char *argv[]) {
  rclcpp::init(argc, argv);
  node = rclcpp::Node::make_shared("raven_bluetooth_joystick_node");
  ...
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] cmd_vel_topic: %s", cmdvel_topic.c_str());
  cmdvel_publisher =
  ...
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] deadzone_percent: %f",
                   deadzone_percent);
  ...
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] device_name: %s",
                   device_name.c_str());
  ...
  if (message_rate < 1) {
    RCUTILS_LOG_FATAL(
        "[raven_bluetooth_joystick_node] message rate must be a positive "
        "integer");
    exit(-1);
  }
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] message_rate: %d",
                   message_rate);

  node->declare_parameter<double>("scale_x", 1.0);
  node->get_parameter("scale_x", scale_x);
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] scale_x: %f", scale_x);

  node->declare_parameter<double>("scale_z", 1.0);
  node->get_parameter("scale_z", scale_z);
  RCUTILS_LOG_INFO("[raven_bluetooth_joystick_node] scale_z: %f", scale_z);
